...
echo ""
echo "<html><head><style>body { font-family: calibri }</style><title>ARK - Stop Server"
echo "</title></head><body>"
echo "<h1>ARK Server Stopping - ${instance}</h1><pre>"
output=`arkmanager stop @${instance} | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g"`
echo "$output"
echo "</pre>"
